...
The `<text>` element draws text inside an SVG element/document. Text has it's own attributes different from circles and ellipses.
...
After enter we aren't chaining any of the other d3 methods!
...
This time we're saving the reference to thos groups in the variable `groups`. 
...
That looks good but it might look better if moved the text around the radius of the circle. Right now the text is at about 3 oclock. Imagine you need to move the text about about 1:30 or 4:30. 

You can use sine and cosine to do this. The circle is divided in radians. Here is a rough map:
...
You'll need to get the raius from the population scale!

The solution to this problem is presented below if you get stuck you can peack it in the next section. 
...
If you need to draw a multi segment line ir a curved line you'll have to use the `path` or `polygon` elements. 
...
- `x1` - x position of the start of of the line
...
Add a line elemet to each group that extends from the center of the circle to text at the edge of the circle. 
...
Here we get the center x value and add cosine of the angle and multiply by the radius of the circle. You could look at it like this: 
...
The path element works a little different other elements. Let's look at what the path looks like in raw SVG form: 
...
In the next step you will draw a simple line. The line will have two straight segments. The first segment will go out from the center to text, essentially the thing you did in the last example, and the next egment will draw a line under the text. 

Here's a picture: 

![challenge-2](images/challenge-2.png)

Here's another picture showing the math. Look closely at this becuase it may be less complicated than you think. 

![example-3](images/example-3.png)

If x1 and y1 are center you can get these numbers same as before. x2 and y2 are also same as the last example. x3 and y3 are the new values. Notice that y3 is same as y2. x3 is x2 plus the length of your line segment. Make things easy and call this 100. 
